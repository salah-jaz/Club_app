...
function SchedulesList() {
  ...
  const reportPreviewCount = useMemo(
    () => filterSchedulesForReport(s.schedules, s.playInvites, report.values).length,
    [s.schedules, s.playInvites, report.values],
  );

  const openSchedulesReport = () => {
    report.openWith({
      status: filters.status !== "all" ? filters.status : "all",
      category: filters.location !== "all" ? filters.location : "all",
      type: "all",
      memberId: "all",
    });
  };

  const exportSchedules = (format: "csv" | "pdf") =>
    runReportExport({
    ...
    <div className="space-y-6">
      ...
      <ReportDialog
        ...
        config={{
          ...
          showDateRange: true,
          showMember: true,
          members: s.members.filter((m) => m.memberType === "adult" || m.memberType === "junior"),
          statusOptions: SCHEDULE_REPORT_STATUS,
          typeOptions: SCHEDULE_REPORT_TYPE,
          categoryOptions: scheduleLocationOptions,
